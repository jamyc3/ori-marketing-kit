...
export function launchPosts(cfg) {
  const l = cfg.launch || {};
  // The link every launch post hands out is the SITE, not the store: it works
  // for everyone (store button + waitlist fallback), it's analytics-counted,
  // and communities like HN respond better to a page than a store listing.
  // Override per-product with cfg.launchUrl; store link only if no domain.
  const url = cfg.launchUrl || (cfg.domain ? `https://${cfg.domain}/` : cfg.appStoreUrl);
